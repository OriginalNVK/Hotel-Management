...
const RoomList = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const normalizedRooms = (rooms || []).map((room) => ({
        ...room,
        Number: room?.Number ?? room?.roomId,
        Type: room?.Type ?? room?.type,
        IsAvailable: room?.IsAvailable ?? room?.isAvailable,
  ...
  useEffect(() => {
    ...
    if (filterType !== 'all') {
      filteredRooms = filteredRooms.filter(
        (room) => String(room?.Type || '').toLowerCase() === filterType
      );
    }
          ...
          <div className="flex gap-2 lg:text-xl text-base font-play">
            ...
            >
              ...
              {roomTypes.map((type, index) => (
                <option
                  key={type?.Type ?? type?.type ?? `type-${index}`}
                  value={String(type?.Type ?? type?.type ?? '').toLowerCase()}
                >
                  {type?.Type ?? type?.type ?? '-'}
                </option>
              ))}
        ...
        <table className="table-auto text-center w-full">
          ...
          <tbody className="lg:text-lg text-base font-amethysta">
            {rooms.map((room, index) => (
              <tr key={room?.Number ?? room?.roomId ?? index}>
